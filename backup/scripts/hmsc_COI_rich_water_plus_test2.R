...
#Check read depths per field replicate before rarefaction
min(sample_sums(DT1)) #4464
max(sample_sums(DT1)) #143064

### Make a table with a column indicating which field replicates have a read depth above the median
readsi<-sample_sums(DT1)
combinedi<-cbind(readsi, sample_data(DT1))
...
sample_data(DT1)$over_median<-combinedi$q[match(sample_data(DT1)$sshc, combinedi$sshc)]

### Extract and then rarefy the PCR replicates with a read depth above the median
above_t<-rarefy_even_depth(subset_samples(DT1, over_median==TRUE), sample.size=as.numeric(threshold), replace=FALSE, trimOTUs = TRUE, rngseed= 13072021)

### Extract the PCR replicates with a read depth at or below the median
below_t<-subset_samples(DT1, over_median==FALSE)
...
#Check read depths per field replicate after rarefaction
...
pdf("diag_beta_COI_rich_p50prevphylum_water_plus_c2_thin10_s1000_tr02.pdf")
plot(mpost$Beta)
dev.off()
