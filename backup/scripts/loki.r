...
mjolnir7_LOKI <- function(lib,min_id = .84){
  ...
  system(paste0("vsearch --usearch_global ",lib,"_SWARM_seeds.fasta --db ",lib,"_SWARM_seeds.fasta --self --id ",min_id," --iddef 1 --userout ",lib,"_match_list.txt -userfields query+target+id --maxaccepts 0 --query_cov .9 --maxhits 10"),intern=T,wait=T)
  ...
  write.table(discarded_db,paste0(lib,"_Discarded_LULU.tsv"),row.names = F,sep="\t",quote = F)
  ...
  write.table(curated_db,paste0(lib,"_Curated_LULU.tsv"),row.names = F,sep="\t",quote = F)
  ...
  write.table(deleted_otu_fate,paste0(lib,"_Deleted_LULU_fate.tsv"),row.names = F,sep="\t",quote = F)
